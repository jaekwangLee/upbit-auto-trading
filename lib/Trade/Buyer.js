...
import {MAX_ONCE_BUY_VALUE, MOVING_AVERAGE_MINUTE, ORDER_TYPE} from '../../constant/trading.js';
...
class BuyTrader extends Observer {
	...
	#getAttractiveMarkets(candleData) {
		...
		for (const [market, candles] of Object.entries(candleData)) {
			...
			const thirtyMinutesMovingAverage = calculateMovingAverage(candles, MOVING_AVERAGE_MINUTE.THIRTY);

			if (isNaN(fiveMinutesMovingAverage) || isNaN(thirtyMinutesMovingAverage)) {
				continue;
			}

			if (fiveMinutesMovingAverage > thirtyMinutesMovingAverage) {
				attractiveMarkets.push({
					market,
					fiveAverageLine: fiveMinutesMovingAverage,
					thirtyAverageLine: thirtyMinutesMovingAverage,
					diffRate: fiveMinutesMovingAverage / thirtyMinutesMovingAverage - 1,
				});
			}
		}
	...
	#getBestMarket(attractiveCandleData = []) {
		...
		const attractiveMarektsState = attractiveCandleData.map((data, index) => `(${index + 1}) ${data.market}: 5평(${digitNum(data.fiveAverageLine, 2)}) | 30평(${digitNum(data.thirtyAverageLine, 2)}) | 변동폭(${data.diffRate})\n`);

		dConsole(`[Buy Trader] 살만한 종목`, ...attractiveMarektsState, `그 중 최선의 선택: ${bestMarket.market}`);
		return bestMarket;
	}
