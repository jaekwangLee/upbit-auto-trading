...
import {MOVING_AVERAGE_MINUTE, MOVING_AVERAGE_MINUTE_BASE_TIME, ORDER_TYPE} from '../../constant/trading.js';
...
class BuyTrader extends Observer {
	...
	#getAttractiveMarkets(candleData) {
		...
		for (const [market, candles] of Object.entries(candleData)) {
			const fiveMinuteCandles = candles.slice(MOVING_AVERAGE_MINUTE_BASE_TIME, MOVING_AVERAGE_MINUTE.FIVE);
			const twentyMinuteCandles = candles.slice(MOVING_AVERAGE_MINUTE_BASE_TIME, MOVING_AVERAGE_MINUTE.TWENTY);
	...
	#getBestMarket(attractiveCandleData = []) {
		...
		const attractiveMarektsState = attractiveCandleData.map((data, index) => `(${index+1}) ${data.market}: 5평(${digitNum(data.fiveAverageLine, 2)}) | 20평(${digitNum(data.twentyAverageLine, 2)}) | 최고가(${digitNum(data.max, 2)}) | 최저가(${digitNum(data.min, 2)})\n`);
