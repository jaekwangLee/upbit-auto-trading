...
class BuyTrader extends Observer {
	...
	#getBestMarket(attractiveCandleData = []) {
		...
		console.log(attractiveCandleData)
		const bestMarket = attractiveCandleData.sort((a, b) => (a.min / a.max > b.min / b.max ? -1 : 1))[0];
		dConsole(`[Buy Trader] 살만한 종목으로는
${attractiveCandleData.map((data, index) => `(${index+1}) ${data.market}: 5평(${digitNum(data.fiveAverageLine, 2)}) | 20평(${digitNum(data.twentyAverageLine, 2)}) | 최고가(${digitNum(data.max, 2)}) | 최저가(${digitNum(data.min, 2)})\n`.replace(',', ''))}
가 있고 그중에 ${bestMarket.market}이 최선의 선택`)
		return bestMarket;
	}
