...
class BuyTrader {
  async run(movingAverageValue, currentValue, balance, ticker) {
    const walletKRW = balance;
    console.log('walletKRW;', walletKRW)
    ...
    console.log('orderPrice, walletKRW: ', orderPrice, walletKRW)
    const volume = getMaxVolume(orderPrice, walletKRW);
    console.log('volume; ', volume);

    if (orderPrice > walletKRW) {
      console.log(`[Buy Trader] No Money What to do... (order: ${orderPrice} / balance: ${walletKRW})`);
      return;
    }

    if (orderPrice * volume < ORDER_PRICE_RULE.MIN) {
      console.log(`[Buy Trader] No money less than minimum orderable price... (orderTotalPrice: ${orderPrice * volume} / tradeMinimumRulePrice: ${ORDER_PRICE_RULE.MIN})`);
      return;
    }
    ...
    console.log(
      `[Buy Trader] ticker: ${ticker} | price: ${currentValue}...`,
      orderResult
    );
  ...
  checkReasonablePrice(averageValue, currentValue) {
    if (averageValue * (1 - MIN_BUY_AVENUE_RATE) > currentValue) {
      console.log(
        `[BUYER] "${currentValue}₩" is reasonable price as "${
