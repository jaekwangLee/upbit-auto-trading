...
class BuyTrader extends Observer {
	...
	#getAttractiveMarkets(candleData) {
		...
		for (const [market, candles] of Object.entries(candleData)) {
			...
			dConsole(`[${market}] : 5평 (${fiveMinutesMovingAverage}) | 20평 (${thirtyMinutesMovingAverage}) }`);
			...
			if (fiveMinutesMovingAverage > thirtyMinutesMovingAverage) {
				attractiveMarkets.push({
					...
					twentyAverageLine: thirtyMinutesMovingAverage,
	...
	#getBestMarket(attractiveCandleData = []) {
		...
		const attractiveMarektsState = attractiveCandleData.map((data, index) => `(${index + 1}) ${data.market}: 5평(${digitNum(data.fiveAverageLine, 2)}) | 20평(${digitNum(data.twentyAverageLine, 2)}) | 상승률(${data.diffRate})\n`);
