...
class BuyTrader {
  async run(movingAverageValue, currentValue, balance, ticker) {
    ...
    console.log(orderPrice, balance, volume);

    if (orderPrice > balance) {
      throw new Error("[Buy Trader] No Money What to do...");
    }

    if (orderPrice * volume < ORDER_PRICE_RULE.MIN) {
      throw new Error("[Buy Trader] No money less than minimum orderable price");
    }
