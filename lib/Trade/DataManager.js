...
import {MAX_GREED_RATE, MAX_LOSS_RATE, MINUTE, MOVING_AVERAGE_MINUTE, ORDER_TYPE, TICKER_REQUEST_TIMING} from '../../constant/trading.js';
...
class TradeDataManager {
	...
	checkOverLoss(currentPrice, boughtPrice) {
		const lossRate = (1 - currentPrice / boughtPrice) * 100;
		return lossRate >= MAX_LOSS_RATE;
	}

	checkOverGreed(currentPrice, boughtPrice) {
		const greedRate = (boughtPrice / currentPrice - 1) * 100;
		return greedRate >= MAX_GREED_RATE;
	}

	getNasdaqChangeRate = async () => {
		try {
			const { data: { data } } = await fetchNasdaqStatus();
			const { primaryData: { percentageChange } } = data;

			return Number(percentageChange.replace('%', ''));
		} catch(error) {
			dConsoleError(`[Data Manager] 나스닥 정보 호출 실패 ${error}`);
			return 0;
		}
	}
	...
	#insertTrade(id, type, reason) {
		return new Promise((resolve, reject) => {
			try {
				const query = type === 1 ? `INSERT INTO trades (bidIdx, type, reason) VALUES (?, ?, ?)` : `INSERT INTO trades (askIdx, type, reason) VALUES (?, ?, ?)`;
