...
class TraderFacade {
  ...
  addTickerHistory(data) {
    ...
    console.log(`collected "${this.ticker}" count: `, this.dataManager.getHistories().length);
    return this;
  }
  ...
  runTradeWorker() {
    this.tradeIntervalId = setInterval(async () => {
      const averageValue = this.dataManager.getMovingAverageValueByMinute(1 * 60 * MILLISECONDS);
      const currValue = this.dataManager.getCurrentValue();
      console.log(`[TRADE WORKER] avg: ${averageValue} | curr: ${currValue}`);
      if (!averageValue || !currValue) {
        return;
      }

      const isGoodPrice = this.buyer.checkReasonablePrice(averageValue, currValue);
      if (!isGoodPrice) {
        return;
      }

      const balance = this.dataManager.getBalance(CURRENCY.KRW);
      const orderResult = await this.buyer.order(this.ticker, balance);
      console.log(`[BUY] ticker: ${this.ticker} | price: ${currValue}...`, orderResult);
    }, 60 * MILLISECONDS);
  }
