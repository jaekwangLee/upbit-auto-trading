...
class TraderFacade {
  ...
  addTickerHistory(data) {
    ...
    console.log("collected tickers: ", this.dataManager.getHistories().length);
    return this;
  }
  ...
  runTradeWorker() {
    this.tradeIntervalId = setInterval(() => {
      const averageValue = this.dataManager.getMovingAverageValueByMinute(
        60 * 5 * MILLISECONDS
      );
      console.log("run trade worker... averageValue: ", averageValue);
    }, 60 * MILLISECONDS);
  }
