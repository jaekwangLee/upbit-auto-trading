...
class TraderFacade {
  constructor(tickers) {
    ...
    this.dataManager = new TradeDataManager(ticker);
  ...
  runTradeWorker() {
    this.tradeIntervalId = setInterval(async () => {
      ...
      // Buy Trader
      this.buyer.run(movingAverage, currValue.price, balance, this.ticker).then((history) => {
        if (history){
          this.dataManager.pushOrderHistory(history);
        }
      });
  ...
  runTradeStateWorker() {
    this.tradeStateIntervalId = setInterval(() => {
