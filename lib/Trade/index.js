...
class TraderManager extends Subject {
	...
	runTickerDataCollector() {
		const getCandles = async () => {
			try {
				...
				const tickers = safeMode === TRADE_MODE.SAFE ? this.fetchPreferMarketCandles() : await this.fetchWarningMarketTickers();
	...
	async fetchWarningMarketTickers() {
		try {
			...
			const tickers = data.filter((marketData) => marketData.market_warning === MARKET_STATUS.WARNING).map((marketData) => marketData.market);
