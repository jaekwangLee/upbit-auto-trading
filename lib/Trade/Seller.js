...
MA 방향성 : ${asset.five > asset.twenty ? '상승' : '하락'} (${asset.five.toLocaleString()} | ${asset.twenty.toLocaleString()})
현재수익률 : ${asset.currValueProfitRate.toFixed(3)}%
보유금액 : ${(asset.volume * asset.price).toFixed(2)}원
손절대상 : ${asset.isOverLoss ? 'O' : 'X'}
손절대상 (이평이탈) : ${asset.five < asset.twenty && asset.price < asset.bought ? 'O' : 'X'}
익절대상 : ${asset.isOverGreed ? 'O' : 'X'}`,
	...
	getSellMarkets(markets, candleData) {
		return markets.filter((market) => {
			...
			const twentyMinuteMovingAverage = calculateMovingAverage(candles, MOVING_AVERAGE_MINUTE.TWENTY);

			if (isNaN(fiveMinutesMovingAverage) || isNaN(twentyMinuteMovingAverage)) {
				return false;
			}

			return fiveMinutesMovingAverage < twentyMinuteMovingAverage && market.trade_price < market.price;
		});
	}
	...
	getAssetStatus(markets, candleData) {
		return markets.map((market) => {
			...
			return {
				...
				twenty: twentyMinuteMovingAverage,
