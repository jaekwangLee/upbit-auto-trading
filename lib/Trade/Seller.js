...
현재수익률(${digitNum(asset.currValueProfitRate)})
손절대상(${asset.isOverLoss}) | 익절대상(${asset.isOverGreed}))`,
	...
	getSellMarkets(markets, candleData) {
		return markets.filter((market) => {
			...
			const averageLineProfitRate = this.coinTradeManager.checkProfitRate(fiveMinutesMovingAverage, twentyMinutesMovingAverage);
	...
	getAssetStatus(markets, candleData) {
		return markets.map((market) => {
			...
			return {
				...
				twenty: twentyMinutesMovingAverage,
