...
import {ORDER_TYPE, MOVING_AVERAGE_MINUTE, STOP_LOSS_RATE} from '../../constant/trading.js';

import {getAverageByNumberField, reduceArrayWithSameKey} from '../../utils/array.js';
...
class SellTrader extends Observer {
	...
	getSellMarkets(_markets, _candleData) {
		return _markets.filter((market) => {
			...
			const fiveMinuteAverageLine = getAverageByNumberField(fiveMinuteCandles, 'min');
			const twentyMinuteAverageLine = getAverageByNumberField(twentyMinuteCandles, 'min');
			if (fiveMinuteAverageLine > twentyMinuteAverageLine) {
				return false;
			}

			return fiveMinuteAverageLine > Number(market.price) * STOP_LOSS_RATE;
		});
	}
	...
	async run(markets = []) {
		...
		const sellTickers = reduceArrayWithSameKey(balances, prices, 'ticker').filter(
			(ticker) => Number(ticker.volume) > 0,
		);
