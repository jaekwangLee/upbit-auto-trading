...
class SellTrader extends Observer {
	...
	async update(dataManager) {
		...
		if (!profitMarkets || profitMarkets.length === 0) {
			...
			const logs = assets.map((asset, index) => `\n(${index + 1}) ${asset.market} - 5평선(${asset.five}원) | 20평선(${asset.twenty}원) | 손절대상(${asset.isOverLoss}) | 익절대상(${asset.isOverGreed}))`);
	...
	getSellMarkets(markets, candleData) {
		return markets.filter((market) => {
			...
			return fiveMinuteAverageLine > twentyMinuteAverageLine;
		});
	}
	...
	getAssetStatus(markets, candleData) {
		return markets.map((market) => {
			...
			const isOverLoss = this.coinTradeManager.checkOverLoss(market.trade_price, market.price);
			const isOverGreed = this.coinTradeManager.checkOverGreed(market.trade_price, market.price);
