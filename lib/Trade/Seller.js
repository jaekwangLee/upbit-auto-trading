...
class SellTrader {
  async run(balances) {
    ...
    const sellTickers = reduceArrayWithSameKey(balances, prices, 'ticker').filter((ticker) => {
      console.log(`[Sell Trader] greed check: ${this.#checkOverMaxGreedPrice(Number(ticker.currentPrice), Number(ticker.price))}`);
      console.log(`[Sell Trader] loss check: ${this.#checkOverMaxLossPrice(Number(ticker.currentPrice), Number(ticker.price))}`);
      return this.#checkOverMaxGreedPrice(Number(ticker.currentPrice), Number(ticker.price)) || this.#checkOverMaxLossPrice(Number(ticker.currentPrice), Number(ticker.price));
    });

    console.log('[Sell Trader] sellTickers: ', sellTickers);
    ...
    console.log('[Sell Trader] orderResult: ', orderResult)
    if (!orderResult) {
      return null;
    }
